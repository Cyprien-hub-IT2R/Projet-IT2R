...
void tache1 (void const *arg)  //PWM
{
  ...
  char etat = 0;
	...
	while(1)
	{
    /*Réception Bluetooth par USART, de la position X et Y du Nunchuk*/
    /*****************************************/
    ...
    /*****************************************/
		...
		Y = recuY - 124;                     //ré-ajustement de Y pour pouvoir traiter l'information sur l'axe Y
		
    recuX=234-recuX+32;                  //afin de tourner le Nunchuk dans le même sens que les roues
...
int main (void) {
  ...
  /*Avant et Arrière de la MCC, Configuration des broches INA et INB*/
...
void Timer_Init(void)                        /*Initialisation du Timer pour créer une PWM (Servomoteurs)*/
	{
...
void TIMER0_IRQHandler(void)                 /*Interruption dès que le Timer atteint le seuil de MR0*/
{
	static int mr0;
	Allumer_1LED(1);                           /*Principe de fonctionnement*/
	LPC_TIM0->IR=1;                            /*Impose un Timer avec un MR0 selon le rapport cyclique décidé*/
	if (etat==0){                              /*Selon l'état, change de niveau afin d'obtenir une PWM*/
			mr0=500000*a;                          /*Etat 0 : Créer le niveau heut de la PWM (mr0 =500000*a), pourcentage de 20 ms entre 5% (1 ms) et 10% (2 ms)*/
			etat=1;                                /*Etat 1 : Créer le niveau bas de la PWM avec la différence selon la période initiale (20 ms - (500000*a))*/
	}                                          
	else if (etat==1){
		mr0=500000-(500000*a);
		etat=0;
...
void Avance_Recule(int a,int x)              /*paramètre a : 1 (avancer) 0 (reculer) et paramètre x : 0-100 (pourcentage de vitesse)*/
{
if (a==1)
{
GPIO_PinWrite(0,16,false);                   //INA     /*Permet de reculer*/
